"""
STANDALONE experiment #4a -- separate from train_loo.py, train_loo_basediff.py, and
train_loo_basediff_joint.py. Only READS (imports, unmodified) data_loader.py, model.py's
HyperNetwork/main_net_forward/main_net_n_params/ARCHITECTURES, and plotting.py's helpers.

Weight-space delta variant (NO regularization), as opposed to the earlier output-space
base+diff experiments. Idea: fit theta_base ONCE (a flat weight vector, same architecture
family as the normal main-net) directly on BASE_KEY's own data via ordinary gradient
optimization -- like training one individual per-CSV model. Freeze it. Then train a
hypernetwork H_comp(Vgsq,Vdsq) -> delta_theta that generates a small CORRECTION to those
SAME weights for every other quiescent point, added directly in weight space:
...
This differs from train_loo_basediff.py/_joint.py, which decomposed in OUTPUT space
(Ids = f_base(Vgs,Vds) + f_diff(...)). Here there is only ONE main-net function throughout;
what's decomposed is its WEIGHT VECTOR. The risk this design has to watch for is the
project's own documented historical failure mode (see train_loo.py's docstring): interpolating
raw weights of INDEPENDENTLY-trained networks gave 39-90% error, because independent training
runs land in arbitrary, mutually-inconsistent neuron permutations/signs. This should NOT
recur here in the same way, because delta_theta is learned via continuous gradient descent
STARTING FROM theta_base (not fit independently then interpolated after the fact) -- so it
has no reason to "jump" to a different permutation, only to move continuously away from it.
...
No penalty on ||delta_theta|| here (see train_loo_deltaweights_reg.py for the regularized
sibling) -- relies only on HyperNetwork's small-last-layer-init to keep deltas modest early on.

Usage:
    python train_loo_deltaweights_noreg.py --csv_dir "C:\\Users\\acost\\repos\\csvs" --architecture arch37
"""
import argparse
import json
import os
...
def _total_loss(hcomp, theta_base, train_keys, tensors, architecture, norm):
    total = 0.0
    for k in train_keys:
        t = tensors[k]
...
def train_hcomp(train_keys, tensors, theta_base, n_params, architecture, epochs, lr, device,
                 lbfgs_epochs=5, lbfgs_max_iter=200, h_hidden=32, h_layers=1, log_every=0, seed=27):
    torch.manual_seed(seed)
    ...
    for epoch in range(epochs):
        opt.zero_grad()
        loss = _total_loss(hcomp, theta_base, train_keys, tensors, architecture, norm)
        loss.backward()
        opt.step()
...
def run_full_fit_job(data, theta_base, n_params, architecture, epochs, lr, lbfgs_epochs,
                      lbfgs_max_iter, h_hidden, h_layers, seed, models_dir, plots_dir):
    ...
    for k in keys:
        ...
        plot_iv_grid(iv_data, k[0], k[1], err,
                     ...
                     title_suffix="(in-sample, full-fit, delta-weights noreg)")
...
def run_loo_job(held_out, data, theta_base, n_params, architecture, epochs, lr, lbfgs_epochs,
    ...
    plot_iv_grid(iv_data, held_out[0], held_out[1], err,
                 ...
                 title_suffix="(held-out, LOO, delta-weights noreg)")
    return held_out, err
...
def main():
    ...
    ap.add_argument("--output_dir",
                     default=r"C:\Users\acost\repos\transistor_modeling_hyper_outputs\results_deltaweights_noreg",
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        full_future = ex.submit(run_full_fit_job, data, theta_base, n_params, architecture, args.epochs,
                                 ...
                                 args.h_layers, args.seed, models_dir, plots_dir)
        loo_futures = {ex.submit(run_loo_job, k, data, theta_base, n_params, architecture, args.epochs,
                                  args.lr, args.lbfgs_epochs, args.lbfgs_max_iter, args.h_hidden,
                                  args.h_layers, args.seed, models_dir, plots_dir): k for k in other_keys}
    ...
    run_info = {
        "pipeline": "train_loo_deltaweights_noreg (standalone)",
    ...
    report_path = build_html_report(args.output_dir, keys, full_errs, loo_errs_padded,
                                     ...
                                     f"{args.architecture} (delta-weights, no reg, base={BASE_KEY})")
